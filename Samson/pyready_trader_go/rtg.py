...
def run(args) -> None:
    ...
    with multiprocessing.Pool(len(args.autotrader) + 2, maxtasksperchild=1) as pool:
        exchange = pool.apply_async(Samson.pyready_trader_go.ready_trader_go.exchange.main,
        ...
        for path in args.autotrader:
            if path.suffix.lower() == ".py":
                pool.apply_async(Samson.pyready_trader_go.ready_trader_go.trader.main, (path.with_suffix("").name,),
